...
def words_sim(word1, word2):
    ...
    if len(intersection) == 0:
        return None
    dice_dist = len(intersection) / (len(w1) + len(w2) - len(intersection))    #2 * len(intersection) / len(union)
...
def getSample(src_file, train_file, test_file, val_file):
    txt = open(src_file, 'r', encoding='utf8').readlines()
    lable_map = {}; label2capacity = {}
    for line in txt:
        for e in line.strip().split('\t')[0].strip().split():
            if e not in lable_map: lable_map[e] = 0
            lable_map[e] += 1
    sorted_lable_map = sorted(lable_map.items(), key=lambda d:d[1], reverse=True)
    lable_set = [e[0] for e in sorted_lable_map if e[1] > 0]
    for le in lable_set:
        for ca in capacity_dict:
            dist = words_sim(ca, le)
            if dist is None: continue
            label2capacity[le + '_' +ca] = dist
    #aa=sorted(label2capacity.items(), key=lambda d:d[1], reverse=True)
    idcnt = 1; res = []
    for line in txt:
        sys.stdout.write('Handle progress: ' + str(idcnt) + ' /  ' + str(len(txt)) + '\n'); sys.stdout.flush(); idcnt += 1
        line_seg = line.strip().split('\t')
        lables = line_seg[0].strip().split()
        qes = line_seg[1]; sim_dict = {}; freq_dict = {}
        for ca in capacity_dict:
            for le in lables:
                if le + '_' + ca in label2capacity:
                    if ca not in sim_dict:
                        sim_dict[ca] = 0
                        freq_dict[ca] = 0
                    sim_dict[ca] += label2capacity[le + '_' +ca]
                    freq_dict[ca] += 1
        sorted_sim_dict = sorted(sim_dict.items(), key=lambda d: d[1], reverse=True)
        sorted_freq_dict = sorted(freq_dict.items(), key=lambda d: d[1], reverse=True)
        if not sorted_sim_dict: continue
        if sorted_sim_dict[0][1] > 0.0 or sorted_freq_dict[0][1] > len(lables) / 2:
            res.append(sorted_sim_dict[0][0] + '\t' + qes + '\n')
    random.shuffle(res)
    with open(train_file, 'w', encoding='utf8') as f1:
        for e in res[:int(len(res) * 0.6)]: f1.write(e)
    with open(test_file, 'w', encoding='utf8') as f2:
...
def get_labels_qes(src_file, des_file):
    txt = open(src_file, 'r').readlines(); idcnt = 1
    fw = open(des_file, 'w')
    for e in txt:
        sys.stdout.write('Handle progress: ' + str(idcnt) + ' /  ' + str(len(txt)) + '\n'); sys.stdout.flush(); idcnt += 1
        split_text = e.split('\t')
        ...
        labels = json.loads((split_text[4]))
        if len(labels) == 0: continue
        label_qes = ' '.join([e[1] for e in labels]) + '\t' + question + '\n'
        fw.write(label_qes)
        # print json.dumps(label_qes, ensure_ascii=False)  ; exit()
    fw.close()
...
#    get_labels_qes('./data/q2.res', './data/lable_qes')
    getSample('./data/label_qes', './data/sen_class.train1', './data/sen_class.test1', './data/sen_class.val1')
