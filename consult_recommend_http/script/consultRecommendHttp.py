...
consultRec = consultRecommend.ConsultRecommend()
...
def task_callback(request, context):
    try:
        ...
        if 'uid' in req_dict['request']['p']:    # 临时的策略
            user_dict['uid'] = req_dict['request']['p']['uid']
        print 'user_dict', json.dumps(user_dict, ensure_ascii=False)
        result = consultRec.run(user_dict)
        ...
        logging.warn('__consult_recommend_failed, req=%s, exception=[%s]' % (json.dumps(req_dict,ensure_ascii=False), str(e)))
...
if __name__== "__main__":
    ...
    server.register_service('consult_recommend', task_callback)
    server.start()
    logging.info('__consult_recommend_server_running__ num_workers: %s' % numworkers)
