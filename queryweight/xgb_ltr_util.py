...
data_path = "get_jdcv_data/"
TASK = "query_weight"
MODEL_FILE = "rank_model/query_weight_xgb.model"
...
def train(model_file):
    ...
    group_train = []
    with open(data_path + TASK + ".train.group", "r", encoding="utf8") as f:
        data = f.readlines()
        for line in data:
            group_train.append(int(line.split("\n")[0]))

    group_valid = []
    with open(data_path + TASK + ".valid.group", "r", encoding="utf8") as f:
        data = f.readlines()
        for line in data:
            group_valid.append(int(line.split("\n")[0]))

    group_test = []
    with open(data_path + TASK + ".test.group", "r", encoding="utf8") as f:
        data = f.readlines()
        for line in data:
    ...
    xgb_model = xgb.train(params, train_dmatrix, num_boost_round=400, evals=[(valid_dmatrix, 'validation')])
