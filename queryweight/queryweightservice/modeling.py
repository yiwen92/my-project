...
def get_attn(q, k, v, attn_mask, d_model, n_head, d_head, dropout, dropatt, is_training, kernel_initializer, residual=True, scope='my_attn', reuse=None):
  """Standard multi-head attention with absolute positional embedding."""
  scale = 1 / (d_head ** 0.5)
  with tf.variable_scope(scope, reuse=reuse):
    # attention heads
...
def rel_multihead_attn(h, r, r_w_bias, r_r_bias, seg_mat, r_s_bias, seg_embed,
  ...
  with tf.variable_scope(scope, reuse=reuse):
    ...
    # core attention ops
    attn_vec, attn_prob = rel_attn_core(
...
def two_stream_rel_attn(h, g, r, mems, r_w_bias, r_r_bias, seg_mat, r_s_bias,
  ...
  with tf.variable_scope(scope, reuse=False):
    ...
    # core attention ops
    attn_vec_h, attn_prob = rel_attn_core(
        q_head_h, k_head_h, v_head_h, k_head_r, seg_embed, seg_mat, r_w_bias,
  ...
  with tf.variable_scope(scope, reuse=True):
    ...
    if target_mapping is not None:
      ...
      attn_vec_g, attn_prob = rel_attn_core(
  ...
  with tf.variable_scope(scope):
    ...
    for i in range(n_layer):
      ...
      with tf.variable_scope('layer_{}'.format(i)):
        if inp_q is not None:
          output_h, output_g, attn_prob = two_stream_rel_attn(
              h=output_h,
              g=output_g,
              r=pos_emb,
          ...
          output_h, attn_prob = rel_multihead_attn(
              h=output_h,
              r=pos_emb,
              r_w_bias=r_w_bias if not untie_r else r_w_bias[i],
              r_r_bias=r_r_bias if not untie_r else r_r_bias[i],
    ...
    with tf.variable_scope("attention", reuse=tf.AUTO_REUSE):
        _seq_len_, _batch_size_ = tf.shape(output)[0], tf.shape(output)[1]
        attention_W = tf.get_variable(name='atention_W', shape=[d_model, d_model], initializer=initializer, dtype=tf_float)
        attention_b = tf.get_variable(name='atention_b', shape=[1, d_model], initializer=initializer, dtype=tf_float)
        attention_output = tf.reshape(output, shape=[-1, d_model])
        attention_embed_wx = tf.matmul(attention_output, attention_W)
        wx_b = tf.nn.relu(attention_embed_wx + attention_b)
        attention_wgh = tf.reshape(tf.reduce_sum(wx_b, 1, keep_dims=True), shape=[_seq_len_, _batch_size_, -1])
        if input_mask is not None:
            _mask_ = input_mask[:,:,None]
            attention_wgh = attention_wgh - 1e30 * _mask_
        attention_out = tf.nn.softmax(attention_wgh, axis=0)
        output = tf.multiply(output, attention_out)  # embeddings * weight_attention
    '''
    bsz = tf.shape(output)[1]
    summary_bias = tf.get_variable('summary_bias', [d_model], dtype=tf_float, initializer=initializer)
    summary_bias = tf.tile(summary_bias[None, None], [1, bsz, 1])
    if input_mask is not None: input_mask = input_mask[None, :, :, None]
    attention_out = get_attn(summary_bias, output, output, input_mask, d_model, n_head, d_head, dropout, dropatt, is_training, initializer, residual=False)
    output = tf.multiply(output, attention_out[0])

    return output, new_mems, lookup_table, attn_prob, attention_out
