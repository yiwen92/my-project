    ...
    for line in tf.gfile.Open(input_path):
      ...
      if not line.strip():
        ...
        if FLAGS.from_raw_text:
          ...
          _, cur_sent = sp.encode_ids(cur_sent)    #sp.encode_ids(cur_sent)   #encode_ids(sp, cur_sent)
        else:
...
def create_tfrecords(save_dir, basename, data, bsz_per_host, seq_len,
  ...
  while i + seq_len <= data_len:
    ...
    for idx in range(bsz_per_host):
      ...
      mask_ = _sample_mask(sp, np.concatenate([a_data, sep_array, b_data, sep_array, cls_array]), reverse=reverse, goal_num_predict=FLAGS.num_predict)
      # concatenate data
      ...
      #print(json.dumps(feature_infos, ensure_ascii=False))
      feature = {
...
def get_input_fn(
  ...
  for idx, record_dir in enumerate(tfrecord_dirs):
    ...
    for filename in cur_record_info["filenames"]:
      basename = os.path.basename(filename)             # ; basename = basename.split("\\")[-1] #;print(basename.split("\\")[-1]);exit()  # linux
