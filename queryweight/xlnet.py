...
class XLNetModel(object):
  ...
  def __init__(self, xlnet_config, run_config, input_ids, seg_ids, input_mask,
    ...
    with tf.variable_scope("model", reuse=tf.AUTO_REUSE):
      (self.output, self.new_mems, self.lookup_table, self.attn_prob, self.attention_out
          ) = modeling.transformer_xl(**tfm_args)
    self.saver = tf.train.Saver(tf.global_variables())          # 保存模型
    self.input_mask = input_mask
    self.initializer = initializer
    self.xlnet_config = xlnet_config
    self.run_config = run_config
    self.output_encode = tf.transpose(self.output, [1, 0, 2])
