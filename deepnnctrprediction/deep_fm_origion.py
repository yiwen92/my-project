...
tf.app.flags.DEFINE_integer("feature_size", 29, "Number of features")       # 117581  # 13
tf.app.flags.DEFINE_integer("field_size", 18, "Number of fields")   # 12
...
tf.app.flags.DEFINE_string("data_dir", './data/test', "data dir")     # criteo_data
tf.app.flags.DEFINE_string("dt_dir", '', "data dt partition")
tf.app.flags.DEFINE_string("model_dir", './criteo_model/DeepFM', "model check point dir")
...
def model_fn(features, labels, mode, params):
    ...
    layers = [int(e) for e in params["deep_layers"].split(',')]         # layers = map(int, params["deep_layers"].split(','))
    dropout = [float(e) for e in params["dropout"].split(',')]          # dropout = map(float, params["dropout"].split(','))
    ...
    #------build f(x)------
    with tf.variable_scope("First-order"):      ##### FM linear part
        feat_wgts = tf.nn.embedding_lookup(FM_W, feat_ids) # None * f/K * K
        y_w = tf.reduce_sum(tf.multiply(feat_wgts, feat_vals),1)        ##### FM <w, x>

    with tf.variable_scope("Second-order"):         ##### FM second part
        ...
        y_v = 0.5*tf.reduce_sum(tf.subtract(sum_square, square_sum),1)	# None * 1      ##### FM sum_sum<vi, vj>(xi, xj)

    with tf.variable_scope("Deep-part"):            ##### embedding deep part
        if FLAGS.batch_norm:
