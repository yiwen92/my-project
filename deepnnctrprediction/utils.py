...
if old == 1:
    ...
    FIELD_SIZES.extend([0] * 5) #= [1] * 12      #FIELD_SIZES = [0] * 26
    FIELD_SIZES.extend([1] * 1)
    FIELD_SIZES.extend([2] * 4)
    FIELD_SIZES.extend([3] * 1)
    FIELD_SIZES.extend([4] * 3)
    FIELD_SIZES.extend([5] * 2)
    FIELD_SIZES.extend([6] * 1)
    FIELD_SIZES.extend([7] * 1)
    FIELD_SIZES.extend([8] * 1)
#FIELD_SIZES = list(range(19))
...
def calScore(labels, predicts, threshold = 0.5):
    assert len(labels) == len(predicts)
    TP = 0;    FP = 0;    FN = 0;    TN = 0
    for i in range(len(labels)):
        if labels[i] == 1:
            if predicts[i] > threshold:  TP += 1     # 11
            else:  FN += 1  # 10
        else:
            if predicts[i] > threshold:    FP += 1  # 01
            else:  TN += 1  # 00
    precision = float(TP) / float(TP + FP + 1e-8);    recall = float(TP) / float(TP + FN + 1e-8)
    acc = float(TP + TN) / float(TP + FN + FP + TN + 1e-8)
...
def read_data(file_name):
    ...
    with open(file_name) as fin:
        for line in fin:
            ...
            y_i = int(fields[0])
            X_i = [int(x.split(':')[0])-1 for x in fields[1:]]
            D_i = [float(x.split(':')[1]) for x in fields[1:]]    #D_i = [int(x.split(':')[1]) for x in fields[1:]]
            y.append(y_i)
            X.append(X_i)
            D.append(D_i)
...
def readdata(file_name, input_dim):
    D = []
    with open(file_name) as fin:
        for line in fin:
            f = [0.0] * input_dim
            ...
            y_i = int(fields[0])
            X_i = [int(x.split(':')[0])-1 for x in fields[1:]]
            D_i = [float(x.split(':')[1]) for x in fields[1:]]
            for i in range(len(X_i)):
                f[X_i[i]] = D_i[i]
            D.append((f, int(fields[0])))
    return D
...
def read_libsvm(file_name):
    ...
    with open(file_name) as fin:
        for line in fin:
            ...
            labels = int(columns[0])
            splits = [e.split(':') for e in columns[1:]]
            feat_ids = [int(e[0]) - 1 for e in splits]
            feat_vals = [float(e[1]) for e in splits]
            Data.append((feat_ids, feat_vals, labels))
    return Data
...
def slice_data(data, start=0, size=-1):
    features = []
    labels = []
    #random.shuffle(data)
    if size == -1 or start + size >= len(data):
        slc_data = data[start:]
    else:
        slc_data = data[start:start + size]
    for e in slc_data:
        features.append(e[0])
        labels.append(e[1])
    return features, labels

def slice_libsvm(data, start=0, size=-1, dense = False, featureNum = None):
    features_id = []
    features_val = []
    labels = []
    #random.shuffle(data)
    if size == -1 or start + size >= len(data):
        ...
        slc_data = data[start:start + size]
    for e in slc_data:
        features_id.append(e[0])
        features_val.append(e[1])
        labels.append(e[2])
    if dense:
        if not featureNum:
            print('slice_libsvm data parameter error !')
            return None
        features = []
        for i in range(len(features_id)):
            feature = [0.0] * featureNum
            for j in range(len(features_id[0])):
                feature[int(features_id[i][j])] = float(features_val[i][j])
            features.append(feature)
        return features, labels
    else:
        return features_id, features_val, labels
